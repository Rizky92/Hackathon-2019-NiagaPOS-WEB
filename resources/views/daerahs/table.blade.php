<table id="file-export" class="table display table-striped table-bordered ">
    <thead>
    <tr class="text-center bg-grey bg-lighten-3 text-dark">
        <th>#</th>
        <th>Nama</th>
        <th>Keterangan</th>
        <th>Parent Daerah</th>
        <th>Action</th>
    </tr>
    </thead>
    <tbody>
    @php
        $no = 1;
    @endphp
    @foreach($daerahs as $daerah)
        <tr>
            <td class="text-center ">{!! $no++ !!}</td>
            <td>{!! $daerah->nama !!}</td>
            <td>{!! $daerah->keterangan !!}</td>
            <td>{!! $daerah->parent['nama'] !!}</td>
            <td class="text-center ">
                {!! Form::open(['route' => ['daerahs.destroy', $daerah->id], 'method' => 'delete']) !!}
                <div class='btn-group'>
                    <a href="{!! route('daerahs.show', [$daerah->id]) !!}" class='btn btn-icon btn-sm btn-outline-success'><i class="fa fa-eye"></i></a>
                    <a href="{!! route('daerahs.edit', [$daerah->id]) !!}" class='btn btn-icon btn-sm btn-outline-warning'><i class="fa fa-pencil"></i></a>
                    {!! Form::button('<i class="fa fa-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-icon btn-sm btn-danger', 'onclick' => "return confirm('Are you sure?')"]) !!}
                </div>
                {!! Form::close() !!}
            </td>
        </tr>
    @endforeach
    </tbody>
</table>
